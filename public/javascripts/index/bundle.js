...
var UpdatePlayerState = function UpdatePlayerState(jsonPlayerState) {
...
var StateChange_SelectingAsADealer = function StateChange_SelectingAsADealer() {
  ...
  (0, _jquery2.default)("#top_Information").text("カードを引いていきましょう");
  (0, _jquery2.default)("#btn_keep_order").text("引く");
...
var StateChange_AnswerCardSelected = function StateChange_AnswerCardSelected(thingCard) {
  if (myState == gameState.Scoring) return;
  myState = gameState.Scoring;
  (0, _jquery2.default)("#top_Information").text("選ばれたのは" + thingCard + "でした。出したのは…");
  (0, _jquery2.default)("#btn_keep_order").hide();
  (0, _jquery2.default)("#btn_keep_order").off('click');
};

var StateChange_SelectingACard = function StateChange_SelectingACard() {
  if (myState == gameState.SelectingACard) return;
  myState = gameState.SelectingACard;
  (0, _jquery2.default)("#top_Information").text("お題に合ったカードを手札から選んでください");
  (0, _jquery2.default)("#btn_keep_order").text("選ぶ");
  (0, _jquery2.default)("#btn_keep_order").show();
  (0, _jquery2.default)("#btn_keep_order").off('click');
  (0, _jquery2.default)("#btn_keep_order").click(function () {
    console.log('onThingCardSelected');
    var selectedID = (0, _jquery2.default)('[name="hand_cards"]:checked').attr('id');
    if (!selectedID) return;
    socket.emit('onThingCardSelected', (0, _jquery2.default)('label[for=' + selectedID + ']').text());
    (0, _jquery2.default)("#btn_keep_order").fadeOut();
  });
  (0, _jquery2.default)("#btn_keep_order").show();
};

var StateChange_PrepairingAGame = function StateChange_PrepairingAGame() {
  if (myState == gameState.PrepairingAGame) return;
  myState = gameState.PrepairingAGame;

  (0, _jquery2.default)("#top_Information").text("全員(2人以上)が準備完了したら、ゲームを開始します");
  (0, _jquery2.default)("#btn_keep_order").text("準備完了");
  (0, _jquery2.default)("#btn_keep_order").off('click');
  (0, _jquery2.default)("#btn_keep_order").click(function () {
  ...
  (0, _jquery2.default)("#description_card").text("");
  ClearHand();
  ClearAnswerArea();
  (0, _jquery2.default)("#btn_keep_order").show();
};
...
var StateChange_WaitingForAllPlayersToSelect = function StateChange_WaitingForAllPlayersToSelect() {
  ...
  (0, _jquery2.default)("#top_Information").text("全員が選び終わるまで待ってください");
  (0, _jquery2.default)("#btn_keep_order").off();
  (0, _jquery2.default)("#btn_keep_order").hide();
};

var UpdateHand = function UpdateHand(privateState) {
...
var HaveAlreadySelected = function HaveAlreadySelected(playerState, socketID) {
  var myPlayerState = MyPlayerState(playerState, socketID);
  return myPlayerState == "選択済み" || myPlayerState == "親";
};

var MyPlayerState = function MyPlayerState(playerState, socketID) {
  var myPlayer = playerState.find(function (player) {
    return player.socketID == socketID;
  });
  if (myPlayer) return myPlayer.state;else return "未参加";
};

var ClearAnswerArea = function ClearAnswerArea() {
  (0, _jquery2.default)(".answer_cards").each(function (index, element) {
...
socket.on('UpdatePlayerStateRequest_Prepair', function (jsonPlayerState) {
  console.log('player state came');
  UpdatePlayerState(jsonPlayerState);
  StateChange_PrepairingAGame();
});

socket.on('UpdatePlayerStateRequest_Select', function (jsonPlayerState) {
  console.log('player state came');
  var playerState = JSON.parse(jsonPlayerState);
  UpdatePlayerState(jsonPlayerState);
  ClearAnswerArea();
...
socket.on('privateGameState', function (jsonPrivateState) {
  console.log('player private state came');
  var privateState = JSON.parse(jsonPrivateState);
  UpdateHand(privateState);
});

socket.on('OnNewDescriptionCard', function (currentDescription) {
  console.log('new description card came');
  (0, _jquery2.default)("#description_card").text(currentDescription);
});

socket.on('OnSelectionPhase', function (dealerSocketID) {
  console.log('Dealer is selecting');
  if (dealerSocketID == socket.id) {
    StateChange_SelectingAsADealer();
  } else {
    (0, _jquery2.default)("#top_Information").text("親がカードを選んでいます");
  }
});

socket.on('DealerDrawACardResult', function (openCards) {
...
socket.on('onAllAnswersDrawn', function (none) {
  console.log('onAllAnswersDrawn');
  (0, _jquery2.default)("#top_Information").text("最も好ましい回答を選んでください");
  (0, _jquery2.default)("#btn_keep_order").text("選ぶ");
  (0, _jquery2.default)("#btn_keep_order").off('click');
  (0, _jquery2.default)("#btn_keep_order").click(function () {
    console.log('onThingCardSelected');
    var selectedID = (0, _jquery2.default)('[name="answer_cards"]:checked').attr('id');
    if (!selectedID) return;
    socket.emit('onAnswerCardSelected', (0, _jquery2.default)('label[for=' + selectedID + ']').text());
    (0, _jquery2.default)("#btn_keep_order").fadeOut();
  });
  (0, _jquery2.default)("#btn_keep_order").show();
});
...
socket.on('onWinningAPoint', function (thingCardName) {
  console.log('onWinningAPoint');
  (0, _jquery2.default)("#btn_keep_order").text("手を挙げる");
  (0, _jquery2.default)("#btn_keep_order").off('click');
  (0, _jquery2.default)("#btn_keep_order").click(function () {
...
socket.on('UpdatePlayerStateRequest_RoundResult', function (jsonPlayerState) {
  (0, _jquery2.default)("#btn_keep_order").hide();
  console.log('player state came');
  var playerState = JSON.parse(jsonPlayerState);
  UpdatePlayerState(jsonPlayerState);
  var winner = playerState.find(function (player) {
    return player.state == "勝者";
  });
  if (winner) {
    (0, _jquery2.default)("#top_Information").text("出したのは" + winner.playerName + "でした。");
  } else {
    (0, _jquery2.default)("#top_Information").text("出したのはコンピューターでした。");
  }
...
socket.on('intrusion_Offer', function (jsonPlayerState) {
  console.log('onWinningAPoint');
  (0, _jquery2.default)("#top_Information").text("ゲームの途中です");
  UpdatePlayerState(jsonPlayerState);
});

socket.on('UpdatePlayerStateRequest_Intrude', function (jsonPlayerState) {
  console.log('乱入成功');
  UpdatePlayerState(jsonPlayerState);
  var playerState = JSON.parse(jsonPlayerState);
  var myPlayerState = MyPlayerState(playerState, socket.id);
